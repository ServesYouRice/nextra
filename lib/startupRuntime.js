...
function redactLogText(value) {
    return String(value)
        ...
        .replace(/(["']?(?:token|secret|credential|password)["']?\s*[:=]\s*)["'][^"']+["']/gi, '$1"[REDACTED]"')
        .replace(/(room\s+)[A-Z0-9]{6}\b/gi, '$1[REDACTED]')
        .replace(/(\/(?:watch|broadcast)\/)[A-Z0-9-]{6,9}\b/gi, '$1[REDACTED]');
}

function installStartupRuntime() {
    const maxLogBytes = Math.max(1024 * 1024, Number.parseInt(process.env.NEXTRA_LOG_MAX_BYTES || '', 10) || 10 * 1024 * 1024);
    const logGenerations = Math.max(1, Math.min(20, Number.parseInt(process.env.NEXTRA_LOG_GENERATIONS || '', 10) || 5));
    ...
    // Synchronous appends keep rotation reliable on Windows, where renaming an
    // open log file fails. Console logging volume is deliberately low.
    const sessionStream = null;
    const latestStream = null;

    const rotateFile = (filePath) => {
        if (!filePath || !fs.existsSync(filePath)) return;
        try {
            for (let generation = logGenerations; generation >= 1; generation--) {
                const source = generation === 1 ? filePath : `${filePath}.${generation - 1}`;
                const destination = `${filePath}.${generation}`;
                if (!fs.existsSync(source)) continue;
                if (generation === logGenerations && fs.existsSync(destination)) fs.unlinkSync(destination);
                fs.renameSync(source, destination);
            }
        } catch {
            // Keep logging to the active file if rotation is unavailable.
        }
    };

    const rotateIfNeeded = (incomingBytes) => {
        const exceeds = [sessionPath, latestPath].some((filePath) => {
            try { return filePath && fs.statSync(filePath).size + incomingBytes > maxLogBytes; } catch { return false; }
        });
        if (!exceeds) return;
        rotateFile(sessionPath);
        rotateFile(latestPath);
    };
    ...
    const append = (chunk, { sync = false } = {}) => {
        ...
        const text = redactLogText(Buffer.isBuffer(chunk) ? chunk.toString('utf8') : String(chunk));
        if (!text) return;
