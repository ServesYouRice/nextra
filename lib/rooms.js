...
function createRoom(hostSocketId, options = {}) {
    ...
    const room = {
        ...
        // OBS / WHIP state
        ingestMode: options.ingestMode || 'browser',
        whipSessionId: null,
        whipResourceId: null,
        whipTransport: null,
        whipProducer: null,
        ...
        relaySupported: options.ingestMode === 'obs',
...
function getRoomStats(code) {
    ...
    return {
        ...
        fallbackViewerCount: (room.fallbackViewers ? Array.from(room.fallbackViewers).filter(id => id !== room.hostSocketId).length : 0),
