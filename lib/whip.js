...
function parseOffer(sdpString) {
    ...
    for (const line of lines) {
        ...
        if (ufragMatch) { currentMedia.iceUfrag = ufragMatch[1].trim(); continue; }

        const pwdMatch = line.match(/^a=ice-pwd:(.+)/);
        if (pwdMatch) { currentMedia.icePwd = pwdMatch[1].trim(); continue; }

        // DTLS fingerprint
        const fpMatch = line.match(/^a=fingerprint:(\S+)\s+(.+)/);
        if (fpMatch) {
            currentMedia.fingerprint = { algorithm: fpMatch[1], value: fpMatch[2].trim() };
            continue;
        }

        // DTLS setup
        const setupMatch = line.match(/^a=setup:(\S+)/);
        if (setupMatch) { currentMedia.setup = setupMatch[1].trim(); continue; }
    ...
    if (result.video) {
        result.video.selectedCodec = selectVideoCodec(result.video);
    }
    if (result.audio) {
...
/**
 * Select the best video codec from offered list. Prefer AV1 > H264 High > H264 Main > H264 Baseline.
 */
function selectVideoCodec(media) {
    // Prefer AV1
    const av1 = media.codecs.find(c => c.name === 'AV1');
    if (av1) return { ...av1, codec: 'av1', profileLevelId: null };

    // Then H264 by profile preference
...
/**
 * Validate that the offered codecs are acceptable.
 */
function validateCodecs(parsedOffer) {
    const warnings = [];

    if (!parsedOffer.video || !parsedOffer.video.selectedCodec) {
        return { valid: false, videoCodec: null, warnings: ['No supported video codec found in offer'] };
    }
    ...
    return { valid: true, videoCodec: vc.codec, warnings };
}
