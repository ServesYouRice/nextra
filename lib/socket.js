...
async function startFallbackRelay(room, mediasoupRouter, io) {
    ...
    // function (after createDirectTransport/consume/createPlainTransport/fps-sample/
    ...
    if (room.fallbackWorker || room.fallbackStarting) return; // already running/starting
    if (!room.whipProducer) return; // no OBS media yet
    room.fallbackStarting = true;
    ...
    try {
        ...
        // Release the synchronous start claim. On success room.fallbackWorker is now
        // set (so the guard still blocks re-entry); on failure it is clear (so a
        // later demand can retry). Either way the "starting" latch must drop.
...
function buildJoinRoomResponse(room) {
            ...
            try {
                ...
                if (existingRoom) {
                    ...
                    if (existingRoom.code === cleanCode && existingRoom.hostSocketId !== socket.id) {
                        ...
                        safeCallback(callback, buildJoinRoomResponse(existingRoom));
                        return;
                    }
                ...
                safeCallback(callback, buildJoinRoomResponse(room));
            } catch (err) {
