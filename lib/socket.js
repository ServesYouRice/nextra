...
function emitHostMetricsSummary(io, summary) {
    ...
    io.to(summary.hostSocketId).emit('room-metrics', {
        ...
        fallbackRestartCount: fullRoom?.fallbackRestartCount || 0,
        fallbackLastError: fullRoom?.fallbackLastError || null,
        fallbackDroppedBytes: fullRoom?.fallbackWorker?.totalStdinDroppedBytes || 0,
...
async function startFallbackRelay(room, mediasoupRouter, io) {
    ...
    if (room.fallbackWorker || (room._mediaPipeline && !room._mediaPipeline.closed)) return;
        ...
        socket.on('reclaim-host', ({ code, hostToken, reloadRecovery = false } = {}, callback) => {
            try {
                ...
                if (reloadRecovery === true && candidateRoom?.reloadRecoveryEnabled !== true) {
                    safeCallback(callback, { success: false, error: 'Reload recovery is not enabled for this room.' });
                    return;
                }
