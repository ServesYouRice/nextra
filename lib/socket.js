...
async function startFallbackRelay(room, mediasoupRouter, io) {
    ...
    try {
        ...
        const relay = new FFmpegRelay({
            ...
            // Audio resumes FALLBACK_AUDIO_BOOTSTRAP_DELAY_MS after the video pipe
            // starts; delay the audio input by the same amount to keep A/V in sync.
            audioOffsetSec: FALLBACK_AUDIO_BOOTSTRAP_DELAY_MS / 1000,
        });
        ...
        console.log(`[Fallback] Video startup backlog ~${backlogSec.toFixed(3)}s (${earlyChunks.length} chunks) for room ${room.code}; audio offset = ${(FALLBACK_AUDIO_BOOTSTRAP_DELAY_MS / 1000 + backlogSec).toFixed(3)}s`);
