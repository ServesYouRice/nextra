...
async function startFallbackRelay(room, mediasoupRouter, io) {
    ...
    try {
        ...
        // Request a keyframe so FFmpeg can start decoding immediately
        ...
        relay.on('fragment', (data) => {
            if (data.sequence <= 3) console.log(`[Fallback] Fragment #${data.sequence} for room ${room.code}: ${data.data?.length} bytes`);
