...
/** Get a free UDP port on 127.0.0.1 by briefly binding and releasing. */
...
function emitHostMetricsSummary(io, summary) {
    ...
    io.to(summary.hostSocketId).emit('room-metrics', {
        ...
        whipConnected: fullRoom?.whipConnected || false,
        obsVideoCodec: fullRoom?.obsVideoCodec || null,
        relayAllowed: fullRoom?.relayAllowed !== false,
        hasRoomTurnServer: fullRoom?.hasRoomTurnServer || false,
        ...
        whepViewerCount: fullRoom?.whepViewerCount || 0,
        totalViewerCount: summary.viewerCount + (fullRoom?.whepViewerCount || 0),
...
function emitAllHostsMetrics(io) {
    ...
    rooms.forEach((room) => emitHostMetricsSummary(io, room));
}
...
function emitToRelayViewers(io, room, event, payload) {
    ...
    for (const viewerSocketId of [...room.relayViewers]) {
        ...
        if (getSocketBufferedBytes(viewerSocket) > config.RELAY_SOCKET_MAX_BUFFERED_BYTES) {
            ...
            viewerSocket.emit('transport-failed', {
                recoverable: true,
                reason: 'Relay connection is too slow. Reconnecting...',
            });
            continue;
        }
...
async function startFallbackRelay(room, mediasoupRouter, io) {
    if (!io) io = registerSocketHandlers._ioRef;
    ...
    try {
        ...
        let lastRtpTs = null;
        let dbgRtpCount = 0;
        // RTP timestamp span (90 kHz video clock) of the frames buffered before the
        // FFmpeg pipe is ready. This is the backlog's content duration; FFmpeg
        // stamps the flushed backlog with synthetic PTS from 0, so the audio input
        // is delayed by this span to keep A/V aligned (otherwise audio runs ahead).
        let backlogFirstTs = null;
        let backlogLastTs = null;
        videoConsumer.on('rtp', (packet) => {
            try {
                ...
                if (relayRef) {
                    if (room.fallbackWorker === relayRef) {
                        dbgRtpCount++;
                    ...
                    if (packet.length >= 8) {
                        const ts = packet.readUInt32BE(4);
                        if (backlogFirstTs === null) backlogFirstTs = ts;
                        backlogLastTs = ts;
                    }
        ...
        // Audio: still delivered to FFmpeg as Opus RTP over UDP (transcoded to AAC).
        let audioEgress = null;
        let audioConsumer = null;
        let ffmpegAudioRtp = 0;
        let ffmpegAudioRtcp = 0;
        if (room.whipAudioProducer) {
            ffmpegAudioRtp = await getFreeUdpPort();
            ffmpegAudioRtcp = await getFreeUdpPort();
            audioEgress = await createPlainTransport(mediasoupRouter);
            await audioEgress.transport.connect({
                ip: '127.0.0.1',
                port: ffmpegAudioRtp,
                rtcpPort: ffmpegAudioRtcp,
            });
        ...
        const relay = new FFmpegRelay({
            ...
            audioRtpPort: ffmpegAudioRtp,
            audioRtcpPort: ffmpegAudioRtcp,
            ...
            // Delay the audio input to line it up with the synthetic-clock video.
            // Tunable (FALLBACK_AUDIO_OFFSET_MS) because the true delay before audio
            // starts is machine-dependent and may exceed the bootstrap timer below;
            // the measured startup video backlog is added on top in start().
        ...
        const clearAudioBootstrapTimer = () => {
            ...
            clearTimeout(audioBootstrapTimer);
        ...
        relay.on('spawn', () => {
            ...
            videoConsumer.requestKeyFrame().catch(() => {});
            // Resume audio shortly after, once FFmpeg has bound its RTP socket.
            clearAudioBootstrapTimer();
            audioBootstrapTimer = setTimeout(() => {
                resumeAudioConsumer();
            }, FALLBACK_AUDIO_BOOTSTRAP_DELAY_MS);
        });
        ...
        relay.on('init', (data) => {
            ...
            io.to(getFallbackAudienceRoom(room.code)).emit('media-init', {
                format: 'fmp4',
                mimeType,
        ...
        relay.on('error', (err) => {
            ...
            if (relay.restartCount >= (require('../config').FALLBACK_RESTART_CAP || 5)) {
                room.fallbackAvailable = false;
                io.to(getFallbackAudienceRoom(room.code)).emit('fallback-error', { reason: 'worker-failed', message: err.message });
            }
        });
        ...
        // Delay the audio input by the buffered video backlog's content duration
        // so the flushed bootstrap frames (synthetic PTS from 0) line up with the
        // live audio that resumes after the bootstrap delay. Without this, audio
        // plays slightly ahead of the video. The 90 kHz video clock can wrap, so
        // use an unsigned 32-bit delta. Clamped to a sane bound to ignore outliers.
        let backlogSec = 0;
        if (backlogFirstTs !== null && backlogLastTs !== null) {
            backlogSec = (((backlogLastTs - backlogFirstTs) >>> 0) / 90000);
            if (!Number.isFinite(backlogSec) || backlogSec < 0 || backlogSec > 5) backlogSec = 0;
        }
        relay.setStartupVideoBacklogSec(backlogSec);
        console.log(`[Fallback] Video startup backlog ~${backlogSec.toFixed(3)}s (${earlyChunks.length} chunks) for room ${room.code}; audio offset = ${(config.FALLBACK_AUDIO_OFFSET_MS / 1000 + backlogSec).toFixed(3)}s (FALLBACK_AUDIO_OFFSET_MS=${config.FALLBACK_AUDIO_OFFSET_MS})`);

        await relay.start();

        // Flush the bootstrap frames buffered during fps detection (keyframe first),
        // then route live frames straight to the relay. FFmpeg assigns constant-rate
        // PTS to all of them, so the burst is timestamped correctly.
        ...
        room._fallbackKfInterval = kfInterval;

        // Store transports for cleanup
        room._fallbackVideoDirect = videoDirect;
        room._fallbackAudioEgress = audioEgress;
        room._fallbackVideoConsumer = videoConsumer;
        room._fallbackAudioConsumer = audioConsumer;

        console.log(`[Fallback] Started relay for room ${room.code} (video via depacketizer pipe)`);
    } catch (err) {
...
function stopFallbackRelay(room) {
    if (room._fallbackKfInterval) {
        clearInterval(room._fallbackKfInterval);
        room._fallbackKfInterval = null;
    }
    ...
    if (room._fallbackVideoDirect) {
        try { room._fallbackVideoDirect.close(); } catch {}
        room._fallbackVideoDirect = null;
    }
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('create-room', (data = {}, callback) => {
            try {
                ...
                if (!existingRoom && getRoomCount() >= config.MAX_ACTIVE_ROOMS) {
                    runtimeMetrics.createRoomDeniedCapacity += 1;
                    safeCallback(callback, { success: false, error: 'Server room capacity reached. Try again later.' });
                    return;
                }
                ...
                const room = createRoom(socket.id, {
                    allowMediaControl: !!data.allowMediaControl,
                    ingestMode,
                    obsAv1Mode,
                    turnConfig,
            ...
            try {
                ...
                safeCallback(callback, {
                    ...
                    metrics: {
                        ...
                        fallbackCodec: room?.fallbackCodec || null,
                        fallbackAvailable: room?.fallbackAvailable || false,
