...
function emitHostMetrics(io, roomCode) {
    ...
    const summary = getRoomStats(roomCode);
    if (!summary) return;
...
async function startFallbackRelay(room, mediasoupRouter, io) {
    ...
    try {
        // Allocate free UDP ports for FFmpeg to receive RTP on.
        // PlainTransport sends consumer RTP to these ports.
        const ffmpegVideoRtp = await getFreeUdpPort();
        const ffmpegVideoRtcp = await getFreeUdpPort();

        const videoEgress = await createPlainTransport(mediasoupRouter);
        await videoEgress.transport.connect({
            ip: '127.0.0.1',
            port: ffmpegVideoRtp,
            rtcpPort: ffmpegVideoRtcp,
        });
        ...
        // Spawn FFmpeg — SDP ports are where FFmpeg listens (the allocated ports)
        const relay = new FFmpegRelay({
            roomCode: room.code,
            videoCodec: room.obsVideoCodec || 'h264',
            hasAudio: !!audioConsumer,
            videoRtpPort: ffmpegVideoRtp,
            videoRtcpPort: ffmpegVideoRtcp,
        ...
        const armInitTimer = () => {
            ...
            initTimer = setTimeout(() => {
                ...
                console.warn(`[Fallback] Init segment timeout for room ${room.code}; restarting relay`);
                relay.restart()
                    .catch((err) => {
        ...
        const resumeVideoAfterSpawn = () => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    if (room.fallbackWorker !== relay) return;
                    videoConsumer.resume().catch((err) => {
                        console.warn(`[Fallback] Failed to resume video consumer for room ${room.code}: ${err.message}`);
                    });
        ...
        relay.on('init', (data) => {
            ...
            recoveringMissingVideoInit = false;
            console.log(`[Fallback] Init segment received for room ${room.code}: ${data.initSegment?.length} bytes, viewers: ${room.fallbackViewers.size}`);
            ...
            // Emit to all fallback viewers (include initSegment so they can set up immediately)
        ...
        // Ask for a keyframe only after FFmpeg has bound its RTP sockets.
        setTimeout(() => {
            if (room.fallbackWorker === relay) {
                videoConsumer.requestKeyFrame().catch(() => {});
            }
        }, 150);

        // Periodically request keyframes until FFmpeg picks one up
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('create-room', (data = {}, callback) => {
            try {
                ...
                if (!existingRoom && getAllRoomStats().length >= config.MAX_ACTIVE_ROOMS) {
        ...
        socket.on('fallback-consume-start', async (data, callback) => {
            ...
            const activeFallbackCount = Array.from(room.fallbackViewers)
                .filter((id) => id !== room.hostSocketId).length;
            if (!room.fallbackViewers.has(socket.id) && activeFallbackCount >= config.MAX_FALLBACK_VIEWERS) {
            ...
            if (!room.fallbackWorker && room.whipProducer) {
