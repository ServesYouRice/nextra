...
async function startFallbackRelay(room, mediasoupRouter, io) {
    ...
    if (room.fallbackCodec !== 'h264') {
        throw new Error(`Fallback relay requires H.264 OBS ingest. Current codec: ${room.fallbackCodec || 'unknown'}`);
    }
    ...
    try {
        ...
        const relay = new FFmpegRelay({
            roomCode: room.code,
            videoCodec: room.fallbackCodec || 'h264',
...
function registerSocketHandlers(io, router) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('create-room', (data = {}, callback) => {
            try {
                ...
                const room = createRoom(socket.id, {
                    ...
                    ingestMode: data.ingestMode === 'obs' ? 'obs' : 'browser',
                });
        ...
        socket.on('fallback-consume-start', async (data, callback) => {
            ...
            if (room.fallbackCodec && room.fallbackCodec !== 'h264') {
