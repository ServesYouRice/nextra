...
class FFmpegRelay extends EventEmitter {
    ...
    async start() {
        ...
        if (this.hasAudio) {
            ...
            this._sdpPath = path.join(os.tmpdir(), `nextra-ffmpeg-${this.roomCode}-${Date.now()}-${crypto.randomBytes(8).toString('hex')}.sdp`);
            fs.writeFileSync(this._sdpPath, sdpContent, { encoding: 'utf-8', mode: 0o600 });
        }
