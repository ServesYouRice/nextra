...
class FFmpegRelay extends EventEmitter {
    ...
    constructor(opts) {
        ...
        this._parser.on('init', (data) => {
            console.log(`[FFmpeg] Parser emitted init for room ${this.roomCode}, forwarding (listeners: ${this.listenerCount('init')})`);
            this.emit('init', data);
        });
        this._parser.on('fragment', (data) => {
            if (data.sequence <= 3) console.log(`[FFmpeg] Parser emitted fragment #${data.sequence} for room ${this.roomCode}`);
    ...
    _buildSdp() {
        const lines = [
            ...
            `a=rtpmap:${this.videoPayloadType} H264/${this.videoClockRate}`,
        ];

        // Add fmtp for codec-specific parameters
        if (this.h264ProfileLevelId) {
            lines.push(`a=fmtp:${this.videoPayloadType} profile-level-id=${this.h264ProfileLevelId};packetization-mode=1`);
        }
    ...
    _buildArgs(sdpPath) {
        ...
        const inputProbeSize = '8000000';
        const inputAnalyzeDuration = '8000000';
        ...
        const args = [
            ...
            // Give FFmpeg enough room to lock onto H.264 SPS/PPS on startup and
            // tolerate modest RTP jitter instead of dropping packets aggressively.
    ...
    async start() {
        ...
        this._process.stdout.on('data', (chunk) => {
            ...
            if (totalStdoutBytes <= chunk.length || totalStdoutBytes % 100000 < chunk.length) {
                console.log(`[FFmpeg:${this.roomCode}] stdout chunk: ${chunk.length} bytes (total: ${totalStdoutBytes})`);
            }
            try {
        ...
        this._process.stderr.on('data', (chunk) => {
            ...
            for (const line of lines) {
                if (line.trim()) {
                    // Console only: stdout/stderr is already mirrored to the
                    // startup log. A synchronous per-line file append here blocks
                    // the event loop on a hot path and can throw if the CWD is
                    // read-only (e.g. the packaged exe).
