...
class FFmpegRelay extends EventEmitter {
    ...
    /**
     * Build SDP content describing the AUDIO RTP stream for FFmpeg input.
     * Video is delivered separately as an Annex-B byte stream over stdin (the
     * relay depacketizes H.264 RTP in Node and injects SPS/PPS), so the SDP only
     * describes the Opus audio that FFmpeg still receives over UDP.
     */
    _buildSdp() {
        const lines = [
            'v=0',
     ...
     * parameter sets. Audio (Opus) is read from the SDP over UDP and transcoded to
     * AAC. Video uses synthetic constant-rate PTS; audio uses its real RTP clock
     * shifted by itsoffset (bootstrap delay + startup video backlog) to align.
     */
    _buildArgs(sdpPath) {
        const inputMaxDelayUs = '500000';
        const inputReorderQueueSize = '2048';
        ...
        if (this.hasAudio) {
            // Input 1: audio — Opus RTP over UDP described by the SDP. Its own RTP
            // timestamps drive timing; FFmpeg normalizes the output to start at zero.
            // itsoffset delays the audio to compensate for two things: the audio
            // bootstrap delay (audio resumes a moment after the video pipe starts)
            // and the startup video backlog (buffered pre-spawn frames get synthetic
            // PTS from 0, i.e. they sit "in the past" relative to the live audio).
            // Without the backlog term audio plays slightly ahead of the video.
            const audioOffset = this.audioOffsetSec + this._startupBacklogSec;
            if (audioOffset > 0) {
                args.push('-itsoffset', audioOffset.toFixed(3));
            }
            args.push(
                '-protocol_whitelist', 'file,udp,rtp',
                '-max_delay', inputMaxDelayUs,
                '-reorder_queue_size', inputReorderQueueSize,
                '-i', sdpPath,
            );
        }
    ...
    async start() {
        ...
        // Write the audio SDP to a temp file (video is fed via stdin, not RTP).
        this._sdpPath = null;
        if (this.hasAudio) {
        ...
        console.log(`[FFmpeg] Starting relay for room ${this.roomCode} (video=pipe Annex-B, audio=${this.hasAudio ? 'sdp' : 'none'})`);
        ...
        this._process = spawn(config.FFMPEG_PATH, args, {
            // stdin = video Annex-B feed, stdout = fMP4 output, stderr = logs.
