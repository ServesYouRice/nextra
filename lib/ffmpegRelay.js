...
class FFmpegRelay extends EventEmitter {
    ...
    _buildSdp() {
        ...
        if (this.videoCodec === 'h264' && this.h264ProfileLevelId) {
            lines.push(`a=fmtp:${this.videoPayloadType} profile-level-id=${this.h264ProfileLevelId};packetization-mode=1`);
        } else if (this.videoCodec === 'av1') {
            // AV1 RTP (RFC 9692) — profile=0 (Main), level-idx=8 (level 4.0 for up to 4K)
            lines.push(`a=fmtp:${this.videoPayloadType} profile=0;level-idx=8;tier=0`);
        }
    ...
    _buildArgs(sdpPath) {
        ...
        const inputProbeSize = this.videoCodec === 'h264' ? '5000000' : '1000000';
        const inputAnalyzeDuration = this.videoCodec === 'h264' ? '5000000' : '1000000';

        const args = [
            '-protocol_whitelist', 'file,udp,rtp',
            // Keep startup reasonably quick without starving RTP video stream detection.
