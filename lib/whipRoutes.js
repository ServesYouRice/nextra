...
function createWhipRouter(mediasoupRouter, options = {}) {
    ...
    const publicEndpoint = options.publicEndpoint === true;
    const getClientIp = typeof options.getClientIp === 'function'
        ? options.getClientIp
        : (req) => req?.socket?.remoteAddress || 'unknown';
    const publicAttempts = new Map();
    let publicPendingStarts = 0;

    const isPublicRateLimited = (ip) => {
        const now = Date.now();
        const current = publicAttempts.get(ip) || {
            count: 0,
            resetAt: now + config.PUBLIC_WHIP_RATE_LIMIT_WINDOW_MS,
        };
        if (now > current.resetAt) {
            current.count = 0;
            current.resetAt = now + config.PUBLIC_WHIP_RATE_LIMIT_WINDOW_MS;
        }
        current.count += 1;
        publicAttempts.set(ip, current);
        return current.count > config.PUBLIC_WHIP_RATE_LIMIT_MAX;
    };
    ...
    router.post(
        ...
        async (req, res) => {
            ...
            let ownsPublicReservation = false;
            let requestCancelled = false;
            const releasePublicReservation = () => {
                if (!ownsPublicReservation) return;
                ownsPublicReservation = false;
                publicPendingStarts = Math.max(0, publicPendingStarts - 1);
            };
            const cancelPendingStart = () => {
                ...
                if (claimedRoom?.whipGeneration === sessionGeneration) {
                    claimedRoom.whipGeneration = null;
                    claimedRoom.whipStarting = false;
                }
                releasePublicReservation();
            };
            req.once('aborted', cancelPendingStart);
            res.once('close', cancelPendingStart);
            try {
                if (!config.WHIP_ENABLED) {
                    setCorsHeaders(res);
                    return res.status(404).json({ error: 'WHIP ingest is not enabled' });
                }
                if (publicEndpoint && !config.PUBLIC_WHIP_ENABLED) {
                    setCorsHeaders(res);
                    return res.status(404).json({ error: 'Public WHIP ingest is not enabled' });
                }
                if (publicEndpoint && isPublicRateLimited(getClientIp(req))) {
                    setCorsHeaders(res);
                    return res.status(429).json({ error: 'Too many public WHIP start attempts' });
                }
                ...
                if (!timingSafeTokenEqual(token, room.hostToken)) {
                    setCorsHeaders(res);
                ...
                if (publicEndpoint) {
                    if (publicPendingStarts >= config.PUBLIC_WHIP_MAX_PENDING_STARTS) {
                        setCorsHeaders(res);
                        return res.status(503).json({ error: 'Public WHIP start capacity reached' });
                    }
                    publicPendingStarts += 1;
                    ownsPublicReservation = true;
                }
                ...
                if (requestCancelled || room.whipGeneration !== sessionGeneration) {
                    transport.close();
                    pendingTransport = null;
                    ...
                    if (dtlsState === 'connected') {
                        try {
                            ...
                            if (room.relayAllowed && codecCheck.videoCodec === 'h264') {
                                startFallbackRelay(room, mediasoupRouter, io)
                ...
                req.off('aborted', cancelPendingStart);
                res.off('close', cancelPendingStart);
