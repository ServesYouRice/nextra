...
class FMP4Parser extends EventEmitter {
    ...
    push(chunk) {
        ...
        const unreadLength = this._bufferLength - this._readOffset;
        const required = unreadLength + chunk.length;
        if (required > this._buffer.length) {
            let capacity = this._buffer.length;
            while (capacity < required) capacity *= 2;
            const grown = Buffer.allocUnsafe(capacity);
            if (unreadLength > 0) this._buffer.copy(grown, 0, this._readOffset, this._bufferLength);
            this._buffer = grown;
            this._bufferLength = unreadLength;
            this._readOffset = 0;
        } else if (this._readOffset > 0 && this._bufferLength + chunk.length > this._buffer.length) {
            this._buffer.copy(this._buffer, 0, this._readOffset, this._bufferLength);
            this._bufferLength = unreadLength;
            this._readOffset = 0;
        }
        chunk.copy(this._buffer, this._bufferLength);
        this._bufferLength += chunk.length;
        this._parseBoxes();
    }

    _parseBoxes() {
        while ((this._bufferLength - this._readOffset) >= 8) {
            const offset = this._readOffset;
            // Read box header: 4 bytes size + 4 bytes type
            let boxSize = this._buffer.readUInt32BE(offset);
            const boxType = this._buffer.readUInt32BE(offset + 4);
            ...
            if (boxSize > 1 && boxSize < 8) {
                this.emit('error', new Error(`MP4 box size ${boxSize} is smaller than header size 8`));
                this._bufferLength = 0;
                this._readOffset = 0;
                break;
            }
            ...
            if (boxSize === 1) {
                if ((this._bufferLength - offset) < 16) break; // need more data for extended header
                // Read 8-byte size (we only support up to Number.MAX_SAFE_INTEGER)
                const hi = this._buffer.readUInt32BE(offset + 8);
                const lo = this._buffer.readUInt32BE(offset + 12);
                boxSize = hi * 0x100000000 + lo;
                if (boxSize < 16) {
                    this.emit('error', new Error(`MP4 extended box size ${boxSize} is smaller than header size 16`));
                    this._bufferLength = 0;
                    this._readOffset = 0;
                    break;
                }
                if (!Number.isSafeInteger(boxSize) || boxSize > MAX_BOX_SIZE) {
                    this.emit('error', new Error(`MP4 extended box size ${boxSize} exceeds limit ${MAX_BOX_SIZE}`));
                    this._bufferLength = 0;
                    this._readOffset = 0;
                    break;
                }
            ...
            if ((this._bufferLength - offset) < boxSize) break;

            // Copy only complete boxes that outlive this queue. Capacity grows
            // geometrically, avoiding repeated copies of an incomplete large box.
            const boxData = Buffer.from(this._buffer.subarray(offset, offset + boxSize));
            this._readOffset += boxSize;

            if (config.MEDIA_DEBUG_LOGS && (this._sequence < 3 || this._state === 'init')) {
        ...
        if (this._readOffset === this._bufferLength) {
            this._readOffset = 0;
            this._bufferLength = 0;
        } else if (this._readOffset > (this._buffer.length / 2)) {
            this._buffer.copy(this._buffer, 0, this._readOffset, this._bufferLength);
            this._bufferLength -= this._readOffset;
            this._readOffset = 0;
        }
    }
